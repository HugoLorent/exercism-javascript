...
export class TranslationService {
  ...
  free(text) {
    return this.api
      .fetch(text)
      .then((translation) => translation.translation)
      .catch((error) => {
        throw error;
      });
  }
  ...
  premium(text, minimumQuality) {
    ...
    const fetchAndValidate = () => {
      return this.api.fetch(text).then(validateTranslation);
    };

    return fetchAndValidate().catch((error) => {
      if (error instanceof NotAvailable) {
        return this.request(text).then(fetchAndValidate);
      }
      throw error;
    });
  }
